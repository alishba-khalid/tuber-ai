...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#C0D3D9]">
        <div className="flex flex-col items-center gap-3">
          <div className="w-8 h-8 rounded-full border-4 border-[#0F6F8A]/30 border-t-[#0F6F8A] animate-spin" />
          <span className="text-xs font-mono-label font-bold text-[#425E67]">LOADING SESSION...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen bg-[#C0D3D9] overflow-hidden">
      
      {/* Sidebar */}
      <aside className="w-60 border-r border-[#9CB4BD] flex flex-col bg-white">
        
        {/* Logo */}
        <div className="p-4 border-b border-[#9CB4BD]">
          <Link href="/" className="flex items-center gap-2.5">
            ...
            <span className="text-lg font-bold font-serif-heading text-[#06151A]">
        ...
        {/* Credits Widget */}
        <div className="p-4 border-b border-[#9CB4BD] bg-white">
          ...
          <div className="text-whitexl font-bold font-serif-heading text-[#06151A] mb-1">
            {credits}
          </div>
          <div className="text-[10px] text-[#68858E] mb-2 font-mono-label">
            FOUNDING ACCESS
          </div>
          <div className="w-full bg-[#9CB4BD] h-1.5 rounded-full overflow-hidden">
            <div 
              ...
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-xl transition-all ${
                  isActive 
                    ...
                    : 'text-[#425E67] hover:text-[#06151A] hover:bg-white'
                }`}
              >
        ...
        <div className="p-3 border-t border-[#9CB4BD]">
          <div className="flex items-center justify-between p-2 rounded-xl hover:bg-white group transition-all">
            <div className="flex items-center gap-2.5">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-xs font-bold text-[#06151A] truncate capitalize">
                  {displayName}
                </div>
                <div className="text-[10px] text-[#68858E] truncate font-mono-label">
                  FREE PLAN
                </div>
              </div>
            </div>
            <button 
              onClick={logout}
              title="Logout" 
              className="p-1 text-[#68858E] hover:text-[#991B1B] transition-colors rounded-lg hover:bg-red-50"
            >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* Top Header */}
        <header className="h-14 border-b border-[#9CB4BD] flex items-center px-6 gap-4 bg-white">
          
          {/* Search bar */}
          <div className="relative flex-1 max-w-md">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#68858E]" />
            <input
              placeholder="Search projects..."
              className="w-full bg-white border border-[#9CB4BD] rounded-xl pl-9 pr-4 py-1.5 text-sm text-[#06151A] placeholder-[#A1A1AA] focus:outline-none focus:border-[#7E9BA6]"
            />
          </div>
          
          {/* Right side icons */}
          <div className="ml-auto flex items-center gap-3">
            <button className="relative p-2 rounded-xl hover:bg-white text-[#68858E] hover:text-[#06151A] transition-all">
