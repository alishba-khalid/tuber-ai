...
export default function Comparison() {
  return (
    <section className="py-20 bg-transparent border-t border-[#CADAE0]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Comparison Table */}
        <div className="bg-[#FFFFFF] border border-[#CADAE0] rounded-2xl overflow-hidden shadow-2xs">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-transparent border-b border-[#CADAE0]">
                  <th className="py-4 px-6 text-xs font-mono-label font-bold text-[#7A9CA8] uppercase">
                    Capability
                  </th>
                  <th className="py-4 px-6 text-sm font-bold font-serif-heading text-[#0F6F8A] bg-[#0F6F8A]/10/60 w-1/3 border-x border-[#CADAE0]">
                    TuberAI
                  </th>
                  <th className="py-4 px-6 text-xs font-mono-label font-bold text-[#7A9CA8] uppercase w-1/3">
                    Other AI Video Tools
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#CADAE0]">
                {comparisons.map((item) => (
                  <tr key={item.feature} className="hover:bg-transparent/50 transition-colors">
                    <td className="py-5 px-6 font-semibold text-sm text-[#0C1D24]">
                      {item.feature}
                    </td>
                    <td className="py-5 px-6 text-sm font-medium text-[#0F6F8A] bg-[#0F6F8A]/10/20 border-x border-[#CADAE0]">
                      <div className="flex items-center gap-2">
                        <Check className="w-4 h-4 text-[#0F6F8A] flex-shrink-0" />
                        <span>{item.tuber}</span>
                      </div>
                    </td>
                    <td className="py-5 px-6 text-sm text-[#7A9CA8]">
                      <div className="flex items-center gap-2">
