      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
          ...
          <div className="lg:col-span-7 space-y-6 text-left">
            ...
            {/* Main Sans Headline */}
            <h1 className="text-4xl sm:text-5xl lg:text-whitexl font-bold font-serif-heading text-[#F9F9F8] leading-[1.12] tracking-tight">
              The ten-hour AI video generator. <span className="text-[#E28C38] block mt-1">Published straight to YouTube.</span>
            </h1>

            {/* Subheadline */}
            <p className="text-sm sm:text-base md:text-lg text-[#A19E95] max-w-2xl font-normal leading-relaxed">
            ...
            <div className="flex flex-wrap gap-2.5 pt-4">
              {subHeroChips.map((chip) => (
                <span
                  key={chip}
                  className="px-3.5 py-1.5 rounded-full text-xs font-medium text-[#A19E95] bg-[#1E1D1A] border border-[#2D2A26]"
                >
                  ✓ {chip}
                </span>
              ))}
          ...
          <div className="lg:col-span-5 w-full">
            <div className="bg-[#1E1D1A] border border-[#2D2A26] rounded-2xl shadow-sm overflow-hidden">
              
              {/* Widget Header Bar */}
              <div className="bg-[#141311] border-b border-[#2D2A26] px-4 py-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="w-2.5 h-2.5 rounded-full bg-[#E28C38]" />
                  <span className="text-[10px] font-mono-label font-bold text-[#F9F9F8] uppercase">
              ...
              {/* Chapter Tabs Row */}
              <div className="flex border-b border-[#2D2A26] bg-[#1E1D1A] overflow-x-auto">
                {chapterTabs.map((tab) => (
                  <button
                    key={tab.id}
                    ...
                    className={`px-3 py-2 text-[10px] font-mono-label font-semibold border-b-2 transition-all flex items-center gap-1.5 flex-shrink-0 ${
                      activeTab === tab.id
                        ? 'border-[#8FE1F4] text-[#E28C38] bg-[#1E1D1A]/60'
                        : 'border-transparent text-[#64748B] hover:text-[#F9F9F8] hover:bg-[#141311]'
                    }`}
                  >
                    {tab.label}
                  </button>
              ...
              <div className="p-4 bg-[#1E1D1A] space-y-4">
                
                {/* Visual Thumbnail */}
                <div className="relative aspect-video rounded-xl overflow-hidden bg-[#141311] border border-[#2D2A26] group">
                  ...
                  >
                    <div className="w-10 h-10 rounded-full bg-[#E28C38] text-[#141311] flex items-center justify-center shadow-md">
                      <Play className="w-4 h-4 fill-current ml-0.5" />
                    </div>
                  </button>

                  <div className="absolute bottom-2.5 left-2.5 z-20 text-[#F9F9F8] text-xs font-medium">
                    <span className="bg-[#E28C38]/20 border border-[#8FE1F4]/40 text-[#E28C38] text-[9px] px-1.5 py-0.5 rounded-full mr-1.5 font-mono-label">SCENE 14</span>
                ...
                <div className="space-y-3">
                  <div>
                    <div className="text-[10px] font-mono-label text-[#E28C38] font-semibold mb-0.5">
                      ACTIVE NARRATION SCRIPT
                    </div>
                    <h3 className="text-sm font-bold text-[#F9F9F8]">
                      {selectedChapter.title}
                    </h3>
                  </div>

                  <p className="text-xs text-[#A19E95] leading-relaxed italic border-l-2 border-[#8FE1F4] pl-3 py-0.5">
                  ...
                  {/* Audio Waveform */}
                  <div className="bg-[#141311] p-3 rounded-xl border border-[#2D2A26] space-y-1.5">
                      ...
                      {[40,65,30,85,50,95,70,45,80,60,90,40,75,55,85,60,40,95,70,50,80,65,90,45,70,85,60,40,75,90,55,70,85,50,60,90,40,75,60,85].map((h, i) => {
                        ...
                        return (
                          <div
                            ...
                            style={{
                              height: `${h}%`,
                              backgroundColor: i < 16 ? '#8FE1F4' : '#2D2A26',
